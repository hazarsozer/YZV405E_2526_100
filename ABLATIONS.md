...
## Round 6 — Caption Enrichment + 3-Channel Blend

Testing whether augmenting captions with idiom-aware context (via Phi-3.5) closes the
visual metaphor gap. Each of the 3,720 unique (caption, compound) pairs received a
single appended sentence describing the abstract concept the image could symbolise for
the idiom — generated by `scripts/enrich_captions.py` (~2.5 hr on RTX 4070 Super).

The new ranker (`scripts/blend_rank.py`) combines three channels via per-instance
min-max normalisation and a weighted sum:

- **C1** = BGE-M3 cos(query, original_caption) — equivalent to `bge_clf_caption`
- **C2** = BGE-M3 cos(query, enriched_caption) — adds idiom-aware context
- **C3** = SigLIP2 cos(siglip_text(query), siglip_image) — direct vision signal

| Strategy | (w₁, w₂, w₃) | Description | Score |
|----------|--------------|-------------|-------|
| `blend_enr_c10.00_c21.00_c30.00` | (0, 1, 0) | Pure enriched captions (C2 only) | 0.33 |
| `blend_enr_c10.40_c20.60_c30.00` | (0.4, 0.6, 0) | C2-dominant blend, no SigLIP2 | 0.34 |
| `blend_enr_c10.20_c20.40_c30.40` | (0.2, 0.4, 0.4) | 3-way blend with SigLIP2 | 0.36 |

**Key finding:** Caption enrichment **hurts**, monotonically. Pure enriched (0.33) is
the worst result of the entire ablation suite — the appended idiom-aware sentence
contaminates the matching signal more than it bridges the metaphor gap. Mixing in the
original caption recovers some lost ground (0.34), and adding SigLIP2 closes most of
the gap to baseline (0.36) but never reaches 0.37.

This is consistent with Round 1: any strategy that mixes in extra signals beyond
`sim(query, original_caption)` ends at or below 0.37. The 0.37 ceiling is robust.
...
| 0.36 | 6 | `qwen_para_caption`, `final_clf`, `final_soft_blend`, `bge_clf_t050`, `bge_clf_t055`, `blend_enr_c10.20_c20.40_c30.40` |
| 0.35 | 3 | `bge_para_caption`, `xenc_clf`, `xenc_para` |
| 0.34 | 1 | `blend_enr_c10.40_c20.60_c30.00` |
| 0.33 | 1 | `blend_enr_c10.00_c21.00_c30.00` |
...
5. **Idiom-aware caption enrichment hurts performance** (Round 6). Augmenting each
   caption with a Phi-3.5-generated sentence describing the symbolic concept dropped
   the score to 0.33 (pure C2). Even blending the enriched captions back with the
   original captions and SigLIP2 (3-way blend) only recovered to 0.36. Negative result:
   the captions cannot be improved enough by an LLM rewrite to clear the 0.37 ceiling.
